...
if __name__ == '__main__':
    df = pd.read_csv("../output/涨跌情况00.csv")
    ...
    for i in range(1,count+1):
        ...
        train_accuracy.append(metrics.accuracy_score(df.loc[:train_count, 'label'], clf.predict(df.loc[:train_count, feature_list])))
        test_accuracy.append(metrics.accuracy_score(df.loc[train_count:, 'label'], clf.predict(df.loc[train_count:, feature_list])))
        print(i,train_accuracy[i-1],test_accuracy[i-1])
    plt.plot(train_accuracy,color='red',label='训练集准确率')
    plt.plot(test_accuracy,color='green',label='测试集准确率')
